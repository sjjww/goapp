FROM golang:1.15-alpine3.12 AS gobuiler-stage

MAINTAINER jewon,woo
LABEL "purpose"="Service Develoyment using Multi-stage builds."

WORKDIR /usr/src/goapp
COPY goapp.go .
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o /usr/local/bin/gostart

FROM scrach AS runtime-stage
